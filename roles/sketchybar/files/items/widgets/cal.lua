...
local function fetch_events_for_day(offset)
    local handle = io.popen("which icalBuddy >/dev/null 2>&1 && icalBuddy -npn -b '' -eep 'url,location,notes' eventsToday || echo 'No events'")
...
return function(sbar)
  local cal_item = Item:new(sbar, "cal", {
    update_freq = 30,
    label = {
        padding_left = 10,
  ...
  -- Toggle popup visibility on mouse enter
  cal:subscribe("mouse.entered", function(env)
      sbar.delay(0.3, function()
          sbar.animate("elastic", 15, function()
              cal:set({
                  icon = { alpha = 1.0, drawing = "toggle", font = { size = 16 }, padding_left = 15, },
                  label = {
                      color = colors.white,
                      font = {
                          style = settings.font.style_map["Bold"],
  ...
  -- Toggle popup visibility on mouse exit
